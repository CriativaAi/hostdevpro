...
class UpdateHostingAccountRequest extends FormRequest
{
    ...
    {
        $account = $this->route('hosting');
        $accountId = $account instanceof HostingAccount ? $account->id : $account;

        return [
            'client_id' => ['required', 'exists:clients,id'],
            'server_id' => ['required', 'exists:servers,id'],
            'domain' => ['required', 'string', 'max:255', 'regex:/^([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,}$/i', Rule::unique('hosting_accounts', 'domain')->ignore($accountId)],
