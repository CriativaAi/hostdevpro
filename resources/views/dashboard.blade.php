<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                ...
                <div class="flex items-center gap-3">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 pt-1">
                ...
                <!-- Botões de Ação Rápida (Menos redondo: rounded-xl) -->
                <div class="flex items-center gap-3">
            ...
            <!-- Banners de Ação Urgente com 80% Transparência e Cantos Refinados (rounded-xl) -->
            @if ($overdueInvoice || $pendingTicket)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                    <!-- Banner 1: Fatura Vencida / Em Aberto -->
                    @if ($overdueInvoice)
                        <div class="p-5 rounded-xl bg-rose-950/40 border border-rose-500/40 flex items-center justify-between gap-4 shadow-xl backdrop-blur-xl">
                            ...
                            <a href="{{ route('invoices.show', $overdueInvoice) }}" 
                               class="px-4 py-2 rounded-lg bg-rose-600 hover:bg-rose-500 text-white font-black text-xs uppercase tracking-wider flex-shrink-0 shadow-lg shadow-rose-600/30 transition">
                                Pagar agora
                            </a>
                        </div>
                    @endif
                    ...
                    @if ($pendingTicket)
                        <div class="p-5 rounded-xl bg-blue-950/40 border border-blue-500/40 flex items-center justify-between gap-4 shadow-xl backdrop-blur-xl">
                            <div class="flex items-center gap-3.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-blue-200/90 mt-0.5 truncate max-w-xs sm:max-w-sm">
                                        #{{ $pendingTicket->ticket_number }} {{ $pendingTicket->subject }}
                                    </p>
                                </div>
                            </div>
                            <a href="{{ route('tickets.show', $pendingTicket) }}" 
                               class="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-wider flex-shrink-0 shadow-lg shadow-blue-600/30 transition">
                                Responder
                            </a>
                        </div>
                    @endif
    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8" x-data="financialTradingDashboard()">
            ...
            <!-- SEÇÃO 1: TICKERS FINANCEIROS (80% Transparência no Branco / Frosted Glass rounded-xl) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                
                <!-- Ticker 1: MRR (Receita Recorrente Mensal) -->
                <div class="p-6 rounded-xl bg-white/[0.06] hover:bg-white/[0.10] backdrop-blur-2xl border border-white/15 shadow-xl relative overflow-hidden group hover:border-emerald-500/50 transition duration-300">
                ...
                <div class="p-6 rounded-xl bg-white/[0.06] hover:bg-white/[0.10] backdrop-blur-2xl border border-white/15 shadow-xl relative overflow-hidden group hover:border-purple-500/50 transition duration-300">
                ...
                <div class="p-6 rounded-xl bg-white/[0.06] hover:bg-white/[0.10] backdrop-blur-2xl border border-white/15 shadow-xl relative overflow-hidden group hover:border-cyan-500/50 transition duration-300">
                ...
                <div class="p-6 rounded-xl bg-white/[0.06] hover:bg-white/[0.10] backdrop-blur-2xl border border-white/15 shadow-xl relative overflow-hidden group hover:border-amber-500/50 transition duration-300">
            ...
            <!-- SEÇÃO 2: GRÁFICOS COLORIDOS DE PERFORMANCE & TRADING (Card Master com 80% Transparência e rounded-2xl) -->
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                
                <!-- Gráfico Master Financeiro / Trading Chart (8 colunas) -->
                <div class="lg:col-span-8 p-6 sm:p-8 rounded-2xl bg-white/[0.06] backdrop-blur-2xl border border-white/15 shadow-2xl space-y-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-white/10 text-xs">
                        <div>
                            <span class="text-slate-400 block text-[11px]">Pico Financeiro:</span>
                            <span class="text-emerald-400 font-black text-sm" x-text="peakFinancial"></span>
                        </div>
                        <div>
                            <span class="text-slate-400 block text-[11px]">Throughput Médio:</span>
                            <span class="text-cyan-400 font-black text-sm" x-text="avgThroughput"></span>
                        </div>
                        <div>
                            <span class="text-slate-400 block text-[11px]">Conversão de Checkout:</span>
                            <span class="text-purple-400 font-black text-sm">94.2%</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block text-[11px]">Uptime do Período:</span>
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="p-6 rounded-2xl bg-white/[0.06] backdrop-blur-2xl border border-white/15 shadow-2xl space-y-4">
            ...
            <div class="bg-white/[0.06] backdrop-blur-2xl border border-white/15 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
                ...
                <!-- Lista de Serviços (rounded-xl) -->
                <div class="space-y-3">
                    @forelse ($services as $service)
                        <div class="p-5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition">
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="px-2.5 py-0.5 rounded text-[10px] font-extrabold border {{ $service->status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-white/[0.05] text-slate-400 border-white/10' }}">
                            ...
                            <div class="flex items-center justify-between sm:justify-end gap-5 border-t sm:border-t-0 border-white/10 pt-3 sm:pt-0">
                                ...
                                <a href="{{ route('hosting.show', $service) }}" 
                                   class="px-4 py-2 rounded-lg bg-white/[0.08] border border-white/15 hover:bg-white/[0.16] text-white font-bold text-xs transition shadow-sm">
                                    Detalhes
                                </a>
                            </div>
                        </div>
            ...
            <div class="bg-white/[0.06] backdrop-blur-2xl border border-white/15 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-blue-500/50 transition group cursor-pointer">
                        <div class="w-8 h-8 rounded-lg bg-blue-500/10 border border-blue-500/30 text-blue-400 flex items-center justify-center mb-2.5">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-purple-500/50 transition group cursor-pointer">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-emerald-500/50 transition group cursor-pointer">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-teal-500/50 transition group cursor-pointer">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-amber-500/50 transition group cursor-pointer">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-rose-500/50 transition group cursor-pointer">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-cyan-500/50 transition group cursor-pointer">
                    ...
                    <div class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-indigo-500/50 transition group cursor-pointer">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white/[0.06] backdrop-blur-2xl border border-white/15 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-4">
                    <div class="flex items-center justify-between border-b border-white/10 pb-3">
                        ...
                        <a href="{{ route('tickets.create') }}" class="px-3 py-1.5 rounded-lg bg-white/[0.08] hover:bg-white/[0.14] border border-white/15 text-white font-bold text-xs transition">
                            Abrir ticket
                        </a>
                    </div>
                    ...
                    <div class="space-y-3">
                        @forelse ($recentTickets as $ticket)
                            <a href="{{ route('tickets.show', $ticket) }}" 
                               class="p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 flex items-center justify-between gap-3 transition block group">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="font-bold text-xs text-white group-hover:text-cyan-400 transition">
                                            #{{ $ticket->ticket_number }} - {{ $ticket->subject }}
                                        </span>
                                        <span class="px-2 py-0.5 rounded text-[10px] font-bold border {{ $ticket->status_badge_classes }}">
                                            {{ $ticket->status_label }}
                                        </span>
                                    </div>
